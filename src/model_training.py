...
from pipeline_config.config import (
    ...
    MODEL_SELECTION_METRIC, MODEL_SELECTION_WEIGHTS, MODEL_CONFIG_FILE, MODEL_EVALUATION_FILE,
    LABEL_MOB
)
...
def save_best_model_config(model_results, model_store_path):
    ...
    """
    # Calculate weighted score if using weighted_score metric
    if MODEL_SELECTION_METRIC == 'weighted_score':
        for result in model_results:
            # Calculate weighted score: 0.5*AUC + 0.3*F1 + 0.2*Precision
            weighted = 0
            for metric, weight in MODEL_SELECTION_WEIGHTS.items():
                weighted += result['val_metrics'].get(metric, 0) * weight
            result['val_metrics']['weighted_score'] = weighted
        
        best_model = max(model_results, key=lambda x: x['val_metrics']['weighted_score'])
    else:
